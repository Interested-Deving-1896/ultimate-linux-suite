#!/bin/bash
#
# Build .rpm package for openSUSE
#
...
BUILD_DIR="$PROJECT_ROOT/build/opensuse"
PKG_NAME="ultimate-linux-suite"
VERSION="$(cat "$PROJECT_ROOT/VERSION" 2>/dev/null || echo "2.3.0")"

echo "==> Building openSUSE .rpm package for $PKG_NAME $VERSION"
...
# Copy spec file
cp "$PROJECT_ROOT/packaging/opensuse/ultimate-linux-suite.spec" "$BUILD_DIR/SPECS/"
